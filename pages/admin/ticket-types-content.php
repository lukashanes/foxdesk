<?php
/**
 * Ticket Types Content (for inclusion in Settings page)
 */

// Handle form submissions for ticket types
    ...
    if (isset($_POST['create_type'])) {
        ...
        if (empty($name)) {
            ...
            db_insert('ticket_types', [
                ...
                'sort_order' => $sort_order, 'is_default' => $is_default, 'is_active' => 1,
...
$type_icons = [
    ...
    'fa-bug' => 'Bug', 'fa-exclamation-triangle' => 'Warning', 'fa-cog' => 'Settings',
    'fa-wrench' => 'Tool', 'fa-lightbulb' => 'Idea', 'fa-comment' => 'Comment',
    'fa-envelope' => 'Email', 'fa-phone' => 'Phone', 'fa-user' => 'User',
];
?>
...
<div class="flex flex-col h-full">
    ...
    <button type="button" class="mb-3 w-full px-3 py-2 rounded-lg text-xs font-medium flex items-center justify-center gap-2 transition-colors"
        ...
        onclick="document.getElementById('add-type-form').classList.toggle('hidden')">
    ...
    <form method="post" id="add-type-form" class="hidden mb-3 p-3 rounded-lg glass-form" style="background: rgba(255, 255, 255, 0.7);">
        ...
        <div class="space-y-2">
            <div>
                ...
                <input type="text" name="name" required class="form-input w-full text-xs" placeholder="Bug, Feature, Support...">
            </div>
            <div>
                <label class="block text-xs mb-1" style="color: var(--text-secondary);">
                    <?php echo e(t('Color')); ?>
            ...
            <div>
                ...
                <div class="grid grid-cols-4 gap-1">
                    ...
                    <label class="relative cursor-pointer" title="<?php echo e($label); ?>">
                        ...
                        <div class="w-6 h-6 flex items-center justify-center rounded border text-xs peer-checked:border-purple-500 peer-checked:bg-purple-50"
                            style="border-color: var(--border-light); color: var(--text-secondary);">
                            <?php echo get_icon($icon, 'w-3 h-3'); ?>
                        </div>
                    </label>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="flex gap-2 pt-1">
                <button type="submit" name="create_type" class="flex-1 btn btn-primary btn-sm text-xs">
                    <?php echo e(t('Create')); ?>
                </button>
                <button type="button" class="flex-1 px-2 py-1 rounded text-xs transition-colors"
                    ...
                    onclick="document.getElementById('add-type-form').classList.add('hidden')">
                    <?php echo e(t('Cancel')); ?>
                </button>
            </div>
        </div>
    ...
    <div id="types-list" class="types-list">
        ...
        <?php else: ?>
            ...
            <div class="type-item accordion-item" data-id="<?php echo $type['id']; ?>"
                data-name="<?php echo e($type['name']); ?>"
                data-color="<?php echo e($type['color']); ?>"
                data-icon="<?php echo e($type['icon']); ?>"
                data-active="<?php echo $type['is_active'] ? '1' : '0'; ?>"
                data-default="<?php echo $type['is_default'] ? '1' : '0'; ?>">
                ...
                <div class="accordion-header">
                    ...
                    <div class="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 text-xs"
                        style="background-color: <?php echo e($type['color']); ?>20; color: <?php echo e($type['color']); ?>">
                        <?php echo get_icon($type['icon'], 'w-3.5 h-3.5'); ?>
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <?php if (!$type['is_active']): ?>
                            <span class="text-xs px-1.5 py-0.5 rounded mt-0.5 inline-block" style="background: rgba(107, 114, 128, 0.1); color: #6b7280;">
                    ...
                    <?php if ($type['is_default']): ?>
                        <span class="text-xs px-1.5 py-0.5 rounded flex-shrink-0" style="background: rgba(139, 92, 246, 0.1); color: #8b5cf6;">
                    ...
                    <form method="post" class="accordion-form">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="pt-2 border-t" style="border-color: var(--border-light);">
                                ...
                                <button type="submit" name="delete_type" class="w-full btn btn-sm text-xs" style="background: rgba(239, 68, 68, 0.1); color: #ef4444;"
                                    onclick="return confirm('<?php echo e(t('Are you sure you want to delete this type?')); ?>')">
                                    <?php echo e(t('Delete')); ?>
                                </button>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (list) {
        new Sortable(list, {
            ...
            forceFallback: true,
            onEnd: function() {
                const items = list.querySelectorAll('.type-item');
